...
T = {
    ...
    "norm": {"ru": "✅ Норма", "kk": "✅ Қалып", "en": "✅ Normal", "tr": "✅ Normal"},
    "dev": {"ru": "❌ Отклонение", "kk": "❌ Ауытқу", "en": "❌ Deviation", "tr": "❌ Sapma"},
    ...
    "today": {"ru": "Сегодня:", "kk": "Бүгін:", "en": "Today:", "tr": "Bugün:"},
    "times": {"ru": "замеры: 08:00 / 14:00 / 20:00", "kk": "өлшемдер: 08:00 / 14:00 / 20:00", "en": "measurements: 08:00 / 14:00 / 20:00", "tr": "ölçümler: 08:00 / 14:00 / 20:00"},
    "day_done": {"ru": "— выполнено", "kk": "— орындалды", "en": "— done", "tr": "— tamam"},
    "day_left": {"ru": "— осталось", "kk": "— қалды", "en": "— left", "tr": "— kaldı"},
    "day_miss": {"ru": "— НЕДОБОР", "kk": "— ОРЫНДАЛМАҒАН", "en": "— SHORTFALL", "tr": "— EKSİK"},
}
...
class PoolPage(ctk.CTkFrame):
    ...
    def pools_card(self):
        ...
        counts = dict(db().execute(
            "SELECT pool_id, COUNT(*) FROM pool_water_records WHERE date=? GROUP BY pool_id",
            (today,)).fetchall())
        ...
        for pid, name, ptype in pools:
            ...
            done = counts.get(pid, 0)
            if done >= 3:
                d_txt, d_col = f"✅ {done}/3 {tt('day_done')}", "#16A34A"
            elif hour >= 20:
                d_txt, d_col = f"🔴 {done}/3 {tt('day_miss')}", "#DC2626"
            else:
                d_txt, d_col = f"⚠️ {done}/3 {tt('day_left')} {3 - done}", "#F59E0B"
            ctk.CTkLabel(row, text=f"{tt('today')} {d_txt} • {tt('times')}",
                         text_color=d_col, anchor="w").pack(side="left", padx=8)
